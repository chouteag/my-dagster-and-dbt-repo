...
@dbt_assets(
    ...
    exclude=INCREMENTAL_SELECTOR # exclude models with INCREMENTAL_SELECTOR
)
...
def incremental_dbt_models(
    ...
    yield from dbt.cli(["build", "--vars", json.dumps(dbt_vars)], context=context).stream() # pass time windows arguments to dbt cli
